class User < ApplicationRecord
  ...
  def User.digest(string)
    cost =
    if ActiveModel::SecurePasssword.min_cost
  ...
  def authenticated?(remember_token)
     ...
     BCrypt::Password.new(remember_digest).is_password?(rememer_token)
  end
  ...
  def self.import(file)
    ...
    CSV.parse(NKF::nkf('-w', File.read(file)), headers: true) do |row|
